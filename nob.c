...
{
    if (!nob_mkdir_if_not_exists_silent("bin"))
        return false;
    if (!nob_mkdir_if_not_exists_silent("initrd"))
        return false;
    if (!nob_mkdir_if_not_exists_silent("initrd/user"))
        return false;
    ...
    if (!go_run_nob_inside(cmd, "kernel"))
        return false;
    if (!go_run_nob_inside(cmd, "user"))
        return false;
    if (!ustar_zip(cmd, "initrd", "bin/iso/initrd"))
        return false;
    return make_iso(cmd);
}
static bool run(Build *build, Cmd *cmd)
{
    ...
    if (build->nographic)
        cmd_append(cmd, "-nographic");
    else
        cmd_append(cmd, "-serial", "stdio");
    if (build->kvm)
        cmd_append(cmd, "-accel", "kvm");
    if (build->cpumax)
        cmd_append(cmd, "-cpu", "max");
    if (build->gdb)
        cmd_append(cmd, "-S", "-s");
    if (build->telmonitor)
    {
...
static void help(const char *exe)
{
    ...
    {
        if (align < strlen(subcmds[i].name))
            align = strlen(subcmds[i].name);
    }
...
int main(int argc, char **argv)
{
    ...
    Cmd cmd = {0};
    if (!bootstrap_submodules(&cmd))
        nob_log(NOB_ERROR, "Failed to bootstrap submodule!");
    if (!file_exists("config.h") && !write_entire_file("config.h", default_config, strlen(default_config)))
        return 1;
    Build build = {0};
    ...
    for (int i = 0; i < argc; ++i)
    {
        if (strcmp(subcmd, "run") == 0 || strcmp(subcmd, "bruh") == 0)
        {
            ...
            else if (strcmp(arg, "-nographic") == 0)
                build.nographic = true;
            else if (strcmp(arg, "-kvm") == 0)
                build.kvm = true;
            else if (strcmp(arg, "-uefi") == 0)
                build.uefi = true;
            else
            {
